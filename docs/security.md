...
**Gateway** (`apps/gateway/src/middleware/security.ts`) — Hono `secureHeaders`:
...
**Dashboard** (`apps/dashboard/next.config.ts` + `middleware.ts`) — the same
...
Leaking one must not unseal the other, so they are never interchangeable — also
...
| A speak-up ciphertext against the phone key | Throws — the keys are not interchangeable |
