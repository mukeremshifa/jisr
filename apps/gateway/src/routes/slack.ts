...
import { audit, findCaseBySlackMessage, findCaseTenantById, repo, withTenant } from '@jisr/db';
import {
  buildEditHoursModal,
  buildRejectModal,
  buildReplyModal,
...
/**
 * Dispatches one decoded interaction payload. Shared by the signed HTTP route and
 * the Socket Mode listener, which receive byte-identical payloads.
 */
export async function handleInteraction(payload: SlackInteraction): Promise<void> {
...
async function resolveActor(input: {
  ...
  // the tenant its private_metadata carried, and a click on a card that is no
  // longer the one of record (a safety mirror, or a site card a later escalation
  // superseded) falls back to the case the button names. The action is still
  // re-validated against the catalog and FGA-checked below.
  const byId = !byCard && !input.companyId ? await findCaseTenantById(input.caseId) : null;
  const companyId = byCard?.companyId ?? input.companyId ?? byId?.companyId;
  if (!companyId) {
